...
# Simple orchestrator stub — discovers PLAN.md and calls node helpers
REPO_ROOT="${1:-.}"
PROJECT_NODE_ID="${2:-}" # optional
DRY_RUN=true
...
while [[ "$#" -gt 0 ]]; do
  case "$1" in
    --repo-root)
      REPO_ROOT="${2:-}"; shift 2;;
    --project-node-id)
      PROJECT_NODE_ID="${2:-}"; shift 2;;
    --dry-run)
      DRY_RUN="${2:-true}"; shift 2;;
    --help)
      echo "Usage: sync-md-to-issues.sh --repo-root . --project-node-id <id> --dry-run true|false";
      exit 0;;
    *) shift ;;
  esac
done
