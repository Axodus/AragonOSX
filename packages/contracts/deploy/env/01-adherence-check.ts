...
/**
 * Reuse-only gate: if there are existing deployments for this network,
 * only allow continuing when the on-chain wiring/permissions are adherent.
 */
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const daoRegistryAddress = daoRegistryDeployment.address;
  const daoFactoryAddress = daoFactoryDeployment.address;
  const pluginRepoRegistryAddress = pluginRepoRegistryDeployment.address;
  const pluginRepoFactoryAddress = pluginRepoFactoryDeployment.address;
  ...
  if (!multisigHasExecute) {
    problems.push(`Multisig sem EXECUTE no ManagementDAO (${multisigAddress})`);
  }
  ...
  if (!daoFactoryHasRegister) {
    problems.push(
      `DAOFactory sem REGISTER_DAO_PERMISSION no DAORegistry (daoFactory=${daoFactoryAddress})`
    );
  }
  ...
  if (!repoFactoryHasRegister) {
    problems.push(
      `PluginRepoFactory sem REGISTER_PLUGIN_REPO_PERMISSION no PluginRepoRegistry (repoFactory=${pluginRepoFactoryAddress})`
    );
  }
